...
const documentSlice = createSlice({
  ...
  reducers: {
    ...
    setActiveDocument: (state, action: PayloadAction<string>) => {
      state.activeDocument = state.documents.find((d) => d.id === action.payload) || null;
    },
  },
});

export const { addDocument, updateDocument, deleteDocument, setActiveDocument } = documentSlice.actions;
